...
extension CSManager {
    ...
    func requestStatements(_ params: BasicRequest, responseHandler: @escaping (CharResponse) -> Void) {
        request(params, host: baseUrl, route: Endpoint.personagens.rawValue, httpMethod: .get) { (jsonData) in
            do {
                let objResponse = try JSONDecoder().decode(CharResponse.self, from: jsonData)
            } catch {
